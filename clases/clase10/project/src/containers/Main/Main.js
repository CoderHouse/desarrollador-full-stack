import React, {Component} from 'react';
import {Route, Switch} from 'react-router-dom';
import {connect} from 'react-redux';

import CreateItem from './CreateItem/CreateItem';
import EditItem from "./EditItem/EditItem";
import ItemCard from '../../components/Item/ItemCard/ItemCard';
import Avatar from '../../components/Avatar/Avatar';

import * as authenticationAction from '../../store/authentication/authentication.action';
import * as itemAction from '../../store/Item/Item.action';
import axios from '../../axios-toDos';
import withErrorHandler from "../../hoc/withErrorHandler";

import './Main.css';
import Spinner from "../../components/UI/Spinner/Spinner";

class Main extends Component {
    state = {
        title: '',
        description: ''
    };
    avatarPlaceholder = "data:image/png;base64,";

    componentDidMount() {
        this.props.onFetchToDos(this.props.user.userName);
    }

    handleEditItem = (itemId) => {
        this.props.history.push(`/app/edit/${itemId}`);
    };

    handleLogoutClick = () => {
        this.props.onRemoveAuthentication();
        this.props.history.push(`/auth`);
    };

    render() {
        let content = <div className="center">
            <Spinner />
        </div>;

        if (!this.props.loadingToDos) {
            content = this.props.toDos.map((item) => (
                <ItemCard
                    key={item.id}
                    title={item.title}
                    loading={item.loading}
                    description={item.description}
                    handleEditClick={() => this.handleEditItem(item.id)}
                    handleDeleteClick={() => this.props.onItemRemoved(item.id)}
                />
            ));
        }

        if (this.props.errorFetchToDos || this.props.errorRemoveItem || this.props.errorAddItem){
            content = <div className="error-form">
                <h1>Something Wrong Happened</h1>
            </div>
        }

        return (
            <div className="Main">
                <div className="main-header">
                    <Avatar url={this.avatarPlaceholder} alias={this.props.user.userName} />
                    <button className="logout-button" onClick={this.handleLogoutClick}>Log out</button>
                </div>
                <div className="main-container">
                    <div className="panel">
                        <Switch>
                            <Route
                                path="/app"
                                exact
                                render={(props) => (
                                    <CreateItem
                                        saveHandler={(item) => this.props.onItemAdded(this.props.user.userName, item)}
                                        {...props} />)}/>

                            <Route path="/app/edit/:id"
                                   exact
                                   render={(props) => {
                                       console.log(props);
                                       return (<EditItem {...props} />)
                                   }}/>
                        </Switch>
                    </div>
                    <div className="toDos-container">
                        {content}
                    </div>
                </div>
            </div>
        );
    }
}

const mapStateToProps = state => {
    return {
        user: state.authentication.user,
        loadingToDos: state.item.loadingToDos,
        toDos: state.item.toDos,
        errorFetchToDos: state.item.errorFetchToDos,
        errorAddItem: state.item.errorAddItem,
        errorRemoveItem: state.item.errorRemoveItem,
    };
};

const mapDispatchToProps = dispatch => {
    return {
        onFetchToDos: (user) => dispatch(itemAction.fetchToDos(user)),
        onItemAdded: (user, item) => dispatch(itemAction.addItem(user, item)),
        onItemRemoved: (itemId) => dispatch(itemAction.removeItem(itemId)),
        onRemoveAuthentication: () => dispatch(authenticationAction.removeAuthentication())
    }
};

export default connect(mapStateToProps, mapDispatchToProps)(withErrorHandler(Main, axios));
