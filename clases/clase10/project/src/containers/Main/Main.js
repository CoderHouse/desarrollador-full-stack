import React, {Component} from 'react';
import {Route, Switch} from 'react-router-dom';
import {connect} from 'react-redux';

import CreateProduct from './CreateProduct/CreateProduct';
import EditProduct from "./EditProduct/EditProduct";
import ProductCard from '../../components/Product/ProductCard/ProductCard';
import Avatar from '../../components/Avatar/Avatar';

import * as authenticationAction from '../../store/authentication/authentication.action';
import * as productAction from '../../store/product/product.action';
import axios from '../../axios-products';
import withErrorHandler from "../../hoc/withErrorHandler";

import './Main.css';
import Spinner from "../../components/UI/Spinner/Spinner";

class Main extends Component {
    state = {
        title: '',
        description: ''
    };
    avatarPlaceholder = "data:image/png;base64,";

    componentDidMount() {
        this.props.onFetchProducts();
    }

    handleEditProduct = (productId) => {
        this.props.history.push(`/app/edit/${productId}`);
    };

    handleLogoutClick = () => {
        this.props.onRemoveAuthentication();
        this.props.history.push(`/auth`);
    };

    render() {
        let content = <div className="center">
            <Spinner />
        </div>;

        if (!this.props.loadingProducts) {
            content = this.props.products.map((product, index) => (
                <ProductCard
                    key={product.id}
                    title={product.title}
                    img={product.img}
                    loading={product.loading}
                    description={product.description}
                    handleEditClick={() => this.handleEditProduct(product.id)}
                    handleDeleteClick={() => this.props.onProductRemoved(product.id)}
                />
            ));
        }

        if (this.props.errorFetchProducts || this.props.errorRemoveProduct || this.props.errorAddProduct){
            content = <div className="error-form">
                <h1>Something Wrong Happened</h1>
            </div>
        }

        return (
            <div className="Main">
                <div className="main-header">
                    <Avatar url={this.avatarPlaceholder} alias={this.props.user.userName} />
                    <button className="logout-button" onClick={this.handleLogoutClick}>Log out</button>
                </div>
                <div className="main-container">
                    <div className="panel">
                        <Switch>
                            <Route
                                path="/app"
                                exact
                                render={(props) => (
                                    <CreateProduct
                                        saveHandler={this.props.onProductAdded}
                                        {...props} />)}/>

                            <Route path="/app/edit/:id"
                                   exact
                                   render={(props) => {
                                       console.log(props);
                                       return (<EditProduct {...props} />)
                                   }}/>
                        </Switch>
                    </div>
                    <div className="products-container">
                        {content}
                    </div>
                </div>
            </div>
        );
    }
}

const mapStateToProps = state => {
    return {
        user: state.authentication.user,
        loadingProducts: state.product.loadingProducts,
        products: state.product.products,
        errorFetchProducts: state.product.errorFetchProducts,
        errorAddProduct: state.product.errorAddProduct,
        errorRemoveProduct: state.product.errorRemoveProduct,
    };
};

const mapDispatchToProps = dispatch => {
    return {
        onFetchProducts: () => dispatch(productAction.fetchProducts()),
        onProductAdded: (product) => dispatch(productAction.addProduct(product)),
        onProductRemoved: (productId) => dispatch(productAction.removeProduct(productId)),
        onRemoveAuthentication: () => dispatch(authenticationAction.removeAuthentication())
    }
};

export default connect(mapStateToProps, mapDispatchToProps)(withErrorHandler(Main, axios));